...
class Category:
	def __init__(self, data):
		...
		scalar = math.sqrt(1/8)
		self.vector = [self.C1, self.C2, self.C3, self.C4, self.C5, self.C6, self.C7, self.C8]
		self.str_vector = map(lambda x: '%.16f'%(x*scalar), self.vector)
		self.sql = 'INSERT INTO diary_vectors VALUES (\'{0}\', cube(ARRAY[{1}]));\n'.format('ptt'+str(self.Did), ', '.join(self.str_vector))
...
def getCategories():
	Categories = []
	for row in  c.execute("SELECT * FROM Article_Category"):
...
def exportCategorySQL(cate, writter):
	writter.write(cate.sql)
...
with open('data/Diary_Vector.sql', 'w') as f:
	for article in Articles:
		i+=1;
		print('Dealing %d out of %d' % (i, len(Articles)))
		exportCategorySQL(article, f)
